...
from floris_cupy.core import Core, State
from floris_cupy.core.rotor_velocity import average_velocity
from floris_cupy.core.turbine.operation_models import (
    POWER_SETPOINT_DEFAULT,
    POWER_SETPOINT_DISABLED,
)
from floris_cupy.core.turbine.turbine import (
    axial_induction,
    power,
    thrust_coefficient,
)
from floris_cupy.cut_plane import CutPlane
...
from floris_cupy.utilities import (
    load_yaml,
    nested_get,
    nested_set,
    print_nested_dict,
)
from floris_cupy.wind_data import (
    TimeSeries,
    WindDataBase,
    WindRose,
    WindRoseWRG,
...
class FlorisModel(LoggingManager):
    ...
    def __init__(self, configuration: dict | str | Path):
        ...
        Args:
            configuration: The Floris configuration dictionary or path to the input YAML file.
                See floris_cupy.default_inputs.yaml for an example of the configuration dictionary
    ...
    def _get_farm_power(
        ...
        """
        Report wind plant power from instance of floris_cupy. Optionally includes
    ...
    def get_expected_farm_power(
        ...
        if len(np.shape(freq)) == 1:
            ...
            return float(np.nansum(np.multiply(freq, farm_power)))
        else:
            weighted_turbine_powers = self._get_weighted_turbine_powers(
                turbine_weights=turbine_weights,
            )
            return float(np.nansum(np.multiply(freq, weighted_turbine_powers)))
    ...
    def get_expected_farm_value(
        ...
        return float(np.nansum(np.multiply(values, farm_power)))
