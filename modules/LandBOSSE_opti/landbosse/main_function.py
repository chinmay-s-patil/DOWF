...
def run_landbosse(Turbine_coordinates, Substation_coordinate, Desired_Voltage, WriteExcel, Display):
    ...
    final_result = manager_runner.run_from_project_list_xlsx(WriteExcel, Display, projects_xlsx, Turbine_coordinates, Substation_coordinate,Desired_Voltage, enable_scaling_study)
    ...
    if WriteExcel:
        ...
        # use function to make nice data frame
        csv_generator = CsvGenerator(file_ops)
        costs = csv_generator.create_costs_dataframe(final_result['module_type_operation_list'])
