    ...
    **Keys in the output dictionary are the following:**
    ...
    def estimate_construction_time(self, estimate_construction_time_input, estimate_construction_time_output):
        ...
        for unit in list_units:
            ...
            material_needs = pd.concat([df for df in [material_needs, unit_quantity] if not df.empty and not df.isna().all().all()])
    ...
    def calculate_costs(self, calculate_cost_input_dict, calculate_cost_output_dict):
        ...
        road_cost = pd.concat([df for df in [road_cost, material_costs, equipment_costs, labor_costs, additional_costs] if not df.empty and not df.isna().all().all()])
