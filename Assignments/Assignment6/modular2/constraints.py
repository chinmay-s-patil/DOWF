...
from config import (
    ...
    ROAD_EXCL_NORMAL_BUFFER, ROAD_EXCL_TOFT_BUFFER, EPSILON,
)
...
def get_seg_data(segs):
    ...
    lensq = cp.where(lensq == 0, EPSILON, lensq)
...
class GPUPenaltyEngine:
    ...
    def __init__(self, site_geom, hh, rotor_diameter, min_spacing,
        ...
        self.road_excl_normal = road_excl_normal if road_excl_normal is not None else rotor_diameter / 2.0 + ROAD_EXCL_NORMAL_BUFFER
        self.road_excl_toft = road_excl_toft if road_excl_toft is not None else rotor_diameter / 2.0 + ROAD_EXCL_TOFT_BUFFER
