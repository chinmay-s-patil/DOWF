...
from config import (
    ...
    KML_SITE_AREA_NAME, KML_ROAD_KREIS_PATTERN, KML_ROAD_STAAT_PATTERN,
    KML_SUBSTATION_PATTERN, KML_BUILDING_PREFIXES, KML_WTG_PREFIX,
    KML_FIELD_BORDERS_NAME, KML_UNTITLED_PATH_NAME, KML_TOWN_COORDS,
)
...
def get_ref(stri):
    """Convert DMS string to local metres relative to reference point."""
...
class SiteGeometry:
    ...
    def _parse_kml(self):
        ...
        for name, pm in _kml_placemarks():
            ...
            if KML_ROAD_KREIS_PATTERN in name or KML_ROAD_KREIS_PATTERN.lower() in name.lower():
                self.road_segments_normal.extend(segs)
            elif KML_ROAD_STAAT_PATTERN in name or KML_ROAD_STAAT_PATTERN.lower() in name.lower():
                self.road_segments_toft.extend(segs)

        self.road_segments = self.road_segments_normal + self.road_segments_toft

        # --- Town ---
        self.town_pos = get_ref(KML_TOWN_COORDS)

        # --- Substation ---
        substation_found = False
        for name, pm in _kml_placemarks():
            if KML_SUBSTATION_PATTERN in name.lower():
        ...
        # --- Buildings / Houses ---
        for name, pm in _kml_placemarks():
            if any(name.startswith(prefix) or name == prefix for prefix in KML_BUILDING_PREFIXES):
                pt = pm.find('kml:Point', _KML_NS)
                if pt is None:
                    continue
                c = _parse_coords(pt.find('kml:coordinates', _KML_NS).text)[0]
        ...
        for name, pm in _kml_placemarks():
            if name.startswith(KML_WTG_PREFIX):
                pt = pm.find('kml:Point', _KML_NS)
                if pt is None:
                    continue
                c = _parse_coords(pt.find('kml:coordinates', _KML_NS).text)[0]
        ...
        fb = []
        for name, pm in _kml_placemarks():
            if name == KML_FIELD_BORDERS_NAME:
        ...
        for name, pm in _kml_placemarks():
            if name == KML_UNTITLED_PATH_NAME:
                ls = pm.find('kml:LineString', _KML_NS)
                if ls is None:
                    continue
                pts = _parse_coords(ls.find('kml:coordinates', _KML_NS).text)
