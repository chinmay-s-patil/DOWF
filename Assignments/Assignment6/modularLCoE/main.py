...
SITE_GEOM = None
WIND_ROSE = None
_times_lock = threading.Lock()   # <-- guard for shared JSON writes
...
def run_optimization(n_turbines, turbine_type_info, seed=42, maxiter=50, disp=True):
    ...
    pi_file = os.path.join(HIST_LOGS_DIR, f"{turbine_type_name}_{n_turbines}_AEP_hist.txt")  # keep filename for monitor compat
    ...
    for fpath in [pen_file, pi_file, cable_file]:
    ...
    def on_new_best(flat_pos, score, pi, penalty, cable_dist, it,
                    cur_pos, cur_score, cur_pi, cur_penalty, cur_cable):
        n_vars = n_turbines + 1
        ...
        title = (f"Iter {it} | Score: {cur_score:.1f} | "
                 f"PI: {cur_pi:.3f} | Pen: {cur_penalty:.3f} | "
        ...
        local_pis.append(cur_pi)
        local_cables.append(cur_cable)

        save_history_plot(
            local_iters, local_pens, local_pis, local_cables, hist_plot_file,
            f"Live History: {n_turbines} Turbines ({turbine_type_name})",
            value_label="PI"
        )

    print(f"Starting PSO for n={n_turbines} for turbine={turbine_type_name}...")
    ...
    # Compute exact PI for the best layout
    pi_engine = PIEngine(turbine_type_info, n_turbines)
    total_cable = sum(np.sqrt((x - best_substation[0])**2 + (y - best_substation[1])**2) for x, y in best_positions)
    best_pi = pi_engine.compute_pi_single(exact_aep_no_yaw / 1e9, total_cable / 1000.0)
...
def save_result_files(result, all_results_dict):
    ...
    pis = [h["pi"] for h in hist]
    cables = [h["cable_dist"] for h in hist]

    save_history_plot(
        iters, pens, pis, cables,
...
def main():
    ...
    print("Running THREAD parallel sweep ...")
    all_results = {}
    ...
    best_overall = max(candidates.values(), key=lambda r: r["pi"])
    ...
    with open(best_file, "w") as f:
        ...
        f.write(f"PI          : {best_overall['pi']:.3f}\n")
    ...
    print(f"\nBest PI      : {best_overall['pi']:.3f}")
    print(f"Best AEP (no yaw): {best_overall['aep_no_yaw'] / 1e9:.2f} GWh/yr")
    if best_overall['aep_yaw'] is not None:
