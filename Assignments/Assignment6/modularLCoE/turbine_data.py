...
from config import TURBINE_LIB, CAPACITY_MIN_MW, CAPACITY_MAX_MW, MIN_SPACING_FACTOR
...
def load_turbine_type(turbine_name):
    ...
    min_spacing = MIN_SPACING_FACTOR * rotor_diameter

    # Rated power in MW
    if 'rated_power' in data:
        p_rated_kw = float(data['rated_power'])
    elif 'power_thrust_table' in data and 'power' in data['power_thrust_table']:
        p_rated_kw = max(float(p) for p in data['power_thrust_table']['power'])
    else:
        p_rated_kw = 3370.0
