...
class YawOptimizationPR(YawOptimization, LoggingManager):
    ...
    def _calculate_farm_power(
        ...
        fm.run()
        turbine_power = fm.get_turbine_powers()
    ...
    def _evaluate_pass_parallel(self, pass_depth):
        ...
        wd_flat = np.tile(self.fmodel_subset.core.flow_field.wind_directions, nturbs * Ny)
        ws_flat = np.tile(self.fmodel_subset.core.flow_field.wind_speeds, nturbs * Ny)
        ti_flat = np.tile(self.fmodel_subset.core.flow_field.turbulence_intensities, nturbs * Ny)
        weights_flat = np.tile(self._turbine_weights_subset, (nturbs * Ny, 1))

        ps = self.fmodel_subset.core.farm.power_setpoints
        ps_flat = np.tile(ps, (nturbs * Ny, 1)) if ps is not None else None
