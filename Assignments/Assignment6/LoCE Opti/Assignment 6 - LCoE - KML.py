...
for lat, lon in wts:
	...
	ax.add_patch(plt.Circle((lon, lat), radius=(HH * 4), facecolor='firebrick', edgecolor='firebrick', alpha=0.5, label="Invalid Area - Wind Turbine" if first_build else "_nolegend_"))
	first_wt = False
...
def run_optimization(n_turbines, seed=42, maxiter=50, disp=True, turbine_type_name=""):
	...
	if SAVE_PLOTS:
		...
		def on_new_best(flat_pos, score, aep, penalty, it,
			...
			cur_aep_gwh = cur_aep / 1e9 if cur_aep > 1e6 else cur_aep
			local_aeps.append(cur_aep_gwh)
